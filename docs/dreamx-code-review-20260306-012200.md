...
最后一次代码变更 (`14e93bf` - 2026-03-04) 已验证：
- 节点选中态阴影优化：`shadow-[0_0_20px_rgba(192,3,28,0.3)]` 精确还原 Drama.Land 发光效果
- 节点内边距微调：`py-3` 使卡片更紧凑
- 表单边框加深：`var(--drama-border-strong)` 增强输入框可见性
...
| 校验项 | 状态 | 验证说明 |
|--------|------|----------|
| 左侧导航栏（悬浮中央） | ✅ | `fixed left-6 top-1/2 -translate-y-1/2` 完美悬浮 |
